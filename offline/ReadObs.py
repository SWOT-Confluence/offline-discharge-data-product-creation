...
def testforfiltering(SWOTts):
    ...
    do_filter=~np.any((np.any(np.isnan(SWOTts['reach/wse'][:].filled(np.nan)[time_filter])))|\
    (np.any(np.isnan(SWOTts['reach/width'][:].filled(np.nan)[time_filter])))|\
    (np.any(np.isnan(SWOTts['reach/slope2'][:].filled(np.nan)[time_filter]))))
    return do_filter
